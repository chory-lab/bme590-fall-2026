...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Paths are sent verbatim by Python; Chromium then flattens every
        # path separator to an underscore for the download name -- so a student
        # passing a folder never gets a file dropped in a directory, it gets a
        # flat name with the path visible in it.
        ("subdir/out.gif", "subdir_out.gif"),  # relative path
        ("subdir/../out.gif", "subdir_.._out.gif"),  # relative with traversal
        ("/abs/path/out.gif", "_abs_path_out.gif"),  # POSIX absolute path
        (r"C:\out\subdir.gif", "C__out_subdir.gif"),  # Windows absolute path
    ],
)
