...
def main():
    ...
    expected |= {"bme590/__init__.py", "bme590/visualizer_ext.py"}
...
def _build_kernel_wheel() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # bme590.visualizer_ext is desktop-only (websocket server + browser
        # tab); the browser build lives in plr_workshops/browser_visualizer_ext.py
        # and is staged here under bme590/ so the workshops' imports resolve
        # unchanged inside Pyodide.
        bme = tmp / "bme590"
        bme.mkdir()
        shutil.copy2(REPO / "plr_workshops" / "browser_visualizer_ext.py", bme / "visualizer_ext.py")
        (bme / "__init__.py").write_text(
            '"""bme590 (browser build): the Pyodide shim of visualizer_ext."""\n',
            encoding="utf-8",
        )
...
def _wheel_contents() -> list:
    ...
    with zipfile.ZipFile(wheel) as z:
        return [n for n in z.namelist()
                if n.startswith("plr_workshops/") or n.startswith("bme590/")]
